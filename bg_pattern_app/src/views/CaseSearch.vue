...
const timelineEvents = computed(() => {
  if (!selectedCase.value || !selectedCase.value.case_details) {
    console.log(
      "timelineEvents: Нет выбранного дела или case_details отсутствуют"
    );
    return [];
  }
...
const showTimelineDialog = (caseData) => {
  console.log(
    "showTimelineDialog: Открытие диалога для дела",
...
const syncWithBitrix = async () => {
  ...
  try {
    ...
    const activities = fetch_data.map((item) => {
      ...
      if (item.case_number)
        commentParts.push(`Номер дела: ${item.case_number}`);
...
onMounted(async () => {
  console.log("Компонент загружен");
   ...
   try {
    ...
    if (userField?.UF_CRM_NUMBER_CASE) {
    ...
    toast.add({
      severity: 'warn',
      summary: 'Информация',
      detail: 'Не удалось автоматически определить номер дела',
      life: 3000,
    });
